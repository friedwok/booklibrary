...
def index(request):
	"""
	Function for displaying homepage of site.
	"""
	num_books=Book.objects.all().count()
	num_instances=BookInstance.objects.all().count()
	# Available books (status = 'a')
	num_instances_available=BookInstance.objects.filter(status__exact='a').count()
	num_authors=Author.objects.count() # Method 'all()' applied by default
	# Number of visits to this view, as counted in the session variable.
	num_visits = request.session.get('num_visits', 0)
	request.session['num_visits'] = num_visits + 1
	# Drawing of html template with data inside variable 'context'
	return render(
		request,
		'index.html',
...
class BookCreate(CreateView):
	...
	def _random_isbn():
		rand_isbn = ''
		for i in range(13):
			rand_isbn += str(random.randint(0, 9))
		return rand_isbn

	initial = {'isbn': _random_isbn(),}
